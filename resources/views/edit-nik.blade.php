        ...
        <form action="{{ route('edit.update', $patient->refId) }}" method="POST">
            ...
            <div class="flex items-start justify-between">
                <div class="leading-relaxed text-sm">
                    <div>Nama</div>
                    <p class="font-semibold bg-lime-100">{{ $patient->identitas->NAMA }}</p>
                    <p class="font-semibold bg-sky-100">{{ $bpjs['response']['peserta']['nama'] }}</p>
                </div>
                ...
                <div class="mb-3 text-sm flex items-center space-x-5">
                    ...
                    <div class="leading-relaxed">
                        ...
                        <p class="bg-lime-100">
                            {{ \Carbon\Carbon::parse($patient->identitas->TANGGAL_LAHIR)->isoFormat('DD MMMM YYYY') }}</p>
                        <p class="bg-sky-100">
            ...
            <div class="flex items-center justify-end space-x-5 border-t py-3">
                <a href="{{ URL::previous() }}">
                    <button type="button" class="px-3 py-1 rounded bg-gray-300 shadow tracking-wide text-sm">Cancel</button>
                </a>
                <button type="submit" class="px-3 py-1 rounded bg-blue-500 text-white shadow tracking-wide text-sm">Update</button>
            </div>
        </form>
    </div>
@endsection

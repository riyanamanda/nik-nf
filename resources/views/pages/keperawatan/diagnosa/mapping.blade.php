<x-layouts.app>
    ...
    <div class="bg-white p-5 rounded shadow border-none mx-auto">
        <div class="mb-5 border p-3 rounded w-full">
            <h1 class="text-sm mb-3">Indikator: <span class="font-medium">{{ $diagnosa->DESKRIPSI }}</span></h1>
            <form action="{{ route('keperawatan.diagnosa.mapping.store', $diagnosa) }}" method="POST">
                @csrf
                ...
                <div class="flex items-center mb-3">
                    <select name="jenis_indikator" id="jenis_indikator" tabindex="1"
                        ...
                        @foreach ($jenis as $item)
                            <option value="{{ $item->ID }}" @selected(old('jenis_indikator') == $item->ID)>{{ $item->DESKRIPSI }}
                            </option>
                        @endforeach
                    </select>
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b-2 border-gray-200">
                <tr>
                    ...
                    <th class="p-3 text-sm font-semibold tracking-wide text-left">Diagnosa</th>
                </tr>
            </thead>
